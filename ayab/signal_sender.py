...
class SignalSender(object):
    ...
    def emit_got_image_flag(self) -> None:
        print("type of self (image)",type(self))
        print("type of self.__signal_receiver (image)",type(self.__signal_receiver))
    ...
    #libby replacing a button presss
    #self.CONFIGURING.addTransition(parent.ui.knit_button.clicked, self.CHECKING)

    def emit_knit_triggered(self) -> None:
        print("type of self (knit)",type(self))
        print("type of self.__signal_receiver (knit)",type(self.__signal_receiver))
#        attributes = dir(self.__signal_receiver)  
#        for attribute in attributes:
#           print(attribute)
        self.__signal_receiver.knit_triggered.emit()
    ...
    def emit_notification(self, message: str = "", log: bool = True) -> None:
        ...
        # libby this is not working with server. not sre why
        print("notification:",message,log)
        self.emit_notifier(message, log)
